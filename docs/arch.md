...
- `"agent"` — 智能体模式，通过 `agent:run` IPC，主进程构建系统提示词、注入工具定义、执行多轮工具循环
...
- 流式 SSE，可中断（`chat:abort` / `agent:abort`）
...
| `chat:check` | 连通性检测 |
| **智能体** | |
| `agent:run` | `{ requestId, messages, agentConfig, apiConfig, temperature? }` |
...
electron/
  ...
  agentService.cjs          # 智能体运行循环：系统提示词构建、工具调度
...
| 主进程无热重载 | `electron/*.cjs` 改动需退出并重新 `make dev` |
...
// 智能体模式（agent:run）— 主进程构建系统提示词并执行工具循环
await api.invoke('agent:run', {
  ...
  agentConfig: settings.agent,  // 完整 AgentConfig（含 soul/user/chatMode/enabledSkillIds…）
  temperature: 0.7,
});
