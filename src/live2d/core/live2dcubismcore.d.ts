declare namespace Live2DCubismCore {
	...
	class Version {
		 ...
		 */
		static csmGetMocVersion(moc: Moc | ArrayBuffer): csmMocVersion;
		private constructor();
	}
	/** Cubism logging. */
	class Logging {
	...
	class Moc {
		...
		/** Native moc. */
		_ptr: number;
		/** Checks moc consistency (Cubism 5+). */
	...
	class Model {
		...
		/** Offscreen renderables (Cubism 5+). */
		...
		/** Updates instance. */
		update(): void;
		/** Returns render orders as array (Cubism 5+). */
	...
	class Parameters {
		...
		/** Parameter values. */
		values: Float32Array;
		/** Parameter types (Cubism 5+). */
		types: Int32Array;
		/** Parameter repeat flags (Cubism 5+). */
		repeats: Uint8Array;
		/**
		 * Initializes instance.
		 *
	...
	class Parts {
		...
		/** Offscreen render target indices per part (Cubism 5+). */
	...
	class Drawables {
		...
		/** Multiply colors (Cubism 5+). */
		multiplyColors: Float32Array;
		/** Screen colors (Cubism 5+). */
		screenColors: Float32Array;
		/** Parent part indices (Cubism 5+). */
		parentPartIndices: Int32Array;
		/** Blend modes per drawable (Cubism 5+). */
	...
	/** Memory management (Cubism 5+). */
	class Memory {
		static initializeAmountOfMemory(bytes: number): void;
		private constructor();
	}
	/** Parameter type enum (Cubism 5+). */
